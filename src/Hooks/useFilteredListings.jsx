...
const useFilteredListings = (currentPage, carCondition, carBrand, carPrice) => {
  ...
  const {
    isPending: filteredListingPending,
    data: filteredListing,
    refetch: filteredListingRefetch,
  } = useQuery({
    queryKey: [
      "filtered-listing",
      currentPage,
    ...
    queryFn: async () => {
      const res = await axiosPublic.get(
        `/filteredListings?listingPerPage=12&currentPage=${currentPage}&carCondition=${carCondition}&carBrand=${carBrand}&carPrice=${carPrice}`
      );

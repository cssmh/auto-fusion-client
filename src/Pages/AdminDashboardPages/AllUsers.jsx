              ...
              <tr key={index}>
                ...
                <td
                  className={`capitalize ${
                    singleUser?.verifyStatus === "verified"
                      ? "text-[#1a9c1a]"
                      : "text-[#f51c1c]"
                  } font-medium`}
                >
                  {singleUser?.verifyStatus}
                </td>

                {/* verification request */}
                <td>
                  {singleUser?.verificationRequest === "requested" &&
                  singleUser?.verifyStatus !== "verified" ? (
                    <div className="flex flex-col justify-center items-center gap-2">
                      <button
                        onClick={() =>
                          handleVerificationRequest(singleUser?._id, "verified")
                        }
                        className="bg-[#088008] text-white px-2 py-1 rounded-[2px] font-medium"
                      >
                        Verify
                      </button>
                      <button
                        onClick={() =>
                          handleVerificationRequest(singleUser?._id, "declined")
                        }
                        className="bg-[#dd1616] text-white px-2 py-1 rounded-[2px] font-medium"
                      >
                        Decline
                      </button>
                    </div>
